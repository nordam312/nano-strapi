...
/**
 * A Plugin (server half) is a self-contained feature, defined as PLAIN DATA.
 * It declares its routes, controllers, services, policies, and lifecycle hooks.
 * The admin (client) half lives in a SEPARATE module (see src/client/types.ts)
 * so server code never bundles React and browser code never bundles node:http —
 * exactly why Strapi splits strapi-server from strapi-admin.
